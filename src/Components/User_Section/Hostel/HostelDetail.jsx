...
function HostelDetail() {
  ...
  useEffect(() => {
    const fetchHostel = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${HOST_API_BASE}/api/public/property/${id}?_=${timestamp}`,
          {
            headers: {
        ...
        if (!found) {
          console.error("No hostel found in response");
          setHostel(null);
          return;
        }
        ...
        // Add a dynamic description if not present
        const fullLocation = `${found.location?.address || ""}, ${
          found.location?.city || ""
        ...
        const updatedHostel = {
          ...
          description:
            found?.description ||
            `${found.name} is a premium hostel located at ${fullLocation}. This property offers ${
              found.totalRooms || 0
            } rooms and ${found.totalBeds || 0} beds, managed by ${
              found.landlord?.name || "the manager"
            }.`,
          postedDays: calculatedPostedDays,
          location: fullLocation,
          manager: {
            name: found.landlord?.name || "N/A",
            contactNumber: found.landlord?.contactNumber || "N/A",
            email: found.landlord?.email || "N/A",
            location: `${found.location?.city || ""}, ${
              found.location?.state || ""
            }`.trim(),
          },
          lowestPrice: found.pricing?.rooms?.min || 0,
        };
    ...
    const fetchReels = async () => {
      ...
      try {
        const timestamp = new Date().getTime();
        const response = await axios.get(
          `${HOST_API_BASE}/api/reels?propertyId=${id}&_=${timestamp}`,
          {
            headers: {
              "Content-Type": "application/json",
            },
  ...
  // Fetch room-specific images after hostel is loaded
  useEffect(() => {
    if (hostel && hostel.rooms) {
      ...
      hostel.rooms.forEach((room) => fetchRoomImages(room.roomId));
    }
  }, [hostel, id]);

  // Fetch bed-specific images after hostel is loaded
  useEffect(() => {
    if (hostel && hostel.rooms) {
  ...
  // Helper for landlord photo
  const getLandlordPhotoUrl = () => {
    if (hostel.landlord?.profilePhoto) {
      const pp = hostel.landlord.profilePhoto;
      return pp.startsWith("http") ? pp : HOST_API_BASE + pp;
    }
    return "/images/landlord-placeholder.png"; // fallback
  };
  ...
  hostel.commonFacilities?.forEach((facility) => {
  ...
  facilityCategories.forEach((category) => {
    const facilities = hostel.facilitiesDetail?.[category];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="relative">
            ...
            {hostel.availability?.hasAvailableRooms && (
              <span className="absolute top-3 left-20 bg-green-500 text-white text-xs font-semibold px-2 py-1 rounded">
                AVAILABLE ({hostel.availability.availableRoomCount} rooms,{" "}
                {hostel.availability.availableBedCount} beds)
              </span>
            )}
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex items-center text-sm text-gray-500 mt-2 gap-3 flex-wrap">
              ...
              <div className="text-sm text-white bg-green-500  rounded-full p-1">
                Posted {postedDays} days ago
              </div>
            </div>

            <p className="text-gray-700 mt-3 text-sm sm:text-base">
              {hostel.shortDescription || hostel.description}
            </p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <InfoItem
              ...
              label="Parking"
              value={
                hostel.availability?.parking ? "Available" : "Not Mentioned"
              }
            />
          </div>
                  ...
                  hostel.rooms.map((room) => {
                    ...
                    room.facilities?.forEach((facility) => {
                      facilityCategories.forEach((category) => {
                        if (facilityKeyMapping[category]?.[facility]) {
                          roomFeatures.push(
                            facilityKeyMapping[category][facility]
                          );
                        }
                      });
                    });
                    ...
                    facilityCategories.forEach((category) => {
                      ...
                      if (facilities && Object.keys(facilities).length > 0) {
                        Object.entries(facilities).forEach(([key, value]) => {
                          if (
                            value === true &&
                            !roomFeatures.includes(
                              facilityKeyMapping[category][key]
                            )
                          ) {
                            roomFeatures.push(
                              facilityKeyMapping[category][key]
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {reels.length > 0 ? (
                  reels.map((reel, i) => (
                    ...
                    >
                      <video
                        src={reel.videoUrl || reel.url || reel.video?.url}
                        controls
                        muted
                        playsInline
                  ...
                  <p className="text-gray-500 text-center col-span-full">
                    No reels available for this hostel.
                  </p>
                )}
              </div>
            )}
...
<div className="mt-6 bg-gray-100 p-4 rounded-xl shadow-md">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <img
                    src={getLandlordPhotoUrl()}
                    alt={hostel.landlord?.name || "Manager"}
                    className="w-16 h-16 rounded-full object-cover"
                  />
                  <div>
                    <p className="font-medium">{hostel.manager.name}</p>
                    <p className="text-sm text-gray-600">
                      {hostel.manager.location}
                    </p>
                    <p className="text-sm text-gray-600">
                ...
                <div className="flex gap-2 w-full sm:w-auto ">
                  <button
                    ...
                    onClick={() =>
                      (window.location.href = `tel:${hostel.manager.contactNumber}`)
                    }
                  >
                  ...
                  <button
                    className="bg-gradient-to-r from-blue-500 via-cyan-400 to-green-400 text-white px-4 py-2 rounded-full flex items-center gap-1 transition flex-1 sm:flex-none"
                    onClick={() =>
                      (window.location.href = `mailto:${hostel.manager.email}`)
                    }
                  >
