...
function ReelsPage() {
  ...
  // Fetch current landlord profile (name + photo from API)
  useEffect(() => {
    async function fetchCurrentLandlord() {
      try {
        const token = localStorage.getItem("token");
        if (!token) return;
        const response = await fetch(
        ...
        const data = await response.json();
        if (data.success) {
          setCurrentLandlord(data.landlord);
        }
      } catch (err) {
        console.error("Profile fetch error:", err);
      }
    }
    fetchCurrentLandlord();
  }, []);
  ...
  const getRemainingFreeReels = () => {
    ...
    const totalUploaded = reels.filter(reel => (reel.landlordId?._id || reel.landlordId) === currentLandlord._id).length;
  ...
  const handleUpload = async (e) => {
    ...
    if (remainingFree <= 0) {
      addNotification("pls buy plans to add more reels", "error");
    ...
    try {
      ...
      const newReel = {
        id: data.reel._id,
        landlordId: currentLandlord?._id,
      ...
      addNotification(`Failed to upload reel: ${error.message}. Please try again.`, "error");
    } finally {
      setUploading(false);
    }
  };
    ...
    <>
      ...
      <div className="min-h-screen w-full bg-black text-white overflow-hidden relative">
        <header className="fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-4 sm:px-6 py-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={`group relative inline-flex items-center gap-2 rounded-full border px-3 py-1.5 transition ${remainingFree <= 0 ? 'border-gray-500 cursor-not-allowed opacity-50' : 'border-white/20 hover:bg-white/10'}`}
                      ...
                      <div className="relative w-[300px] aspect-[9/16] bg-neutral-900 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10">
                        ...
                        <div className="absolute top-3 left-3 flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {currentLandlord && currentLandlord._id === (reel.landlordId?._id || reel.landlordId) && (
                              <button
                        ...
                        <div className="absolute left-3 right-20 bottom-3 space-y-1 pr-2">
                          ...
                          <div className="mt-1 flex items-center gap-1 text-[11px] text-white/80">
                            ...
                            <span>
                              Original audio • {formatCompact(reel.likes)} likes
                            </span>
                          </div>
                        </div>
                        <div className="absolute right-2 bottom-4 flex flex-col items-center gap-4">
        ...
        {showUpload && (
          ...
          .max-h-96:hover::-webkit-scrollbar-thumb {
            background: #ffffff33;
            border-radius: 4px;
          }
          select option {
        ...
        `}
                  ...
                  <div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && form.tagsInput) {
                            e.preventDefault();
                            setForm((f) => ({
                              ...f,
                              tags: [...(f.tags || []), f.tagsInput.trim()],
                              tagsInput: "",
                            }));
                          }
                        }}
                        placeholder="e.g. property, garden, outdoor"
                        className="mt-1 w-full rounded-lg border border-white/15 bg-black/30 p-2"
                      />
                      {form.tags && form.tags.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {form.tags.map((tag, index) => (
                            <span
                              key={index}
                              className="inline-flex items-center px-2 py-1 bg-white/20 text-white rounded-full text-sm"
                            >
                              #{tag}
                              <button
                                type="button"
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  ...
                  <button
                    disabled={uploading || remainingFree <= 0}
                    type="submit"
