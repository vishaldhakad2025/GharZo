...
import { FaCheckCircle, FaChartLine, FaShieldAlt, FaHeadset, FaLock, FaCrown, FaTimes, FaBed, FaVideo, FaTimes as FaClose } from 'react-icons/fa';

const SubscriptionPlans = () => {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('beds'); // 'beds' or 'reels'
    ...
    fetch('https://api.gharzoreality.com/api/landlord/profile', {
      ...
      .then((profileData) => {
        if (profileData.success) {
          const id = profileData.landlord._id;
          setLandlordId(id);
    ...
    fetch(url, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          setError(`Failed to fetch ${activeTab} plans`);
        }
        setLoading(false);
      })
      .catch((err) => {
        setError(`Error fetching ${activeTab} plans`);
        setLoading(false);
      });
  };
  ...
  const handleMyPlansClick = () => {
    if (landlordId) {
      navigate(activeTab === 'beds' 
        ? `/landlord/my-subscriptions/${landlordId}` 
        : `/landlord/my-reel-subscriptions`);
    } else {
  ...
  const handleSubscribe = async (plan) => {
    ...
    try {
      ...
      const price = activeTab === 'beds' ? plan.price : plan.pricePerReel;
  ...
  const getPlanFields = (plan) => {
    if (activeTab === 'beds') {
      return {
        duration: formatDuration(plan.durationInDays),
        max: plan.maxBeds,
  ...
  const isPopular = (plan) => {
    if (activeTab === 'beds') {
      return plan.price === Math.max(...plans.map(p => p.price));
    } else {
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-[#00C2FF] to-[#00FFAA] bg-clip-text text-transparent mb-4">
              Subscription Plans - {activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}
            </h1>
            <p className="text-lg text-gray-300 max-w-xl mx-auto leading-relaxed">
              Unlock the full potential of your {activeTab} business with our tailored subscription plans. Choose wisely and grow smarter.
            </p>
          </div>
          ...
          <div className="flex justify-center mb-8 space-x-4">
            <button
              ...
              className={`px-6 py-3 rounded-full font-semibold flex items-center gap-2 transition duration-300 ${
                activeTab === 'beds'
                  ? 'bg-gradient-to-r from-[#00C2FF] to-[#00FFAA] text-gray-900 shadow-lg'
