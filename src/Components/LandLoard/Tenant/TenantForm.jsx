  ...
  useEffect(() => {
    const fetchLandlordId = async () => {
      ...
      try {
        ...
        toast.error("Failed to fetch landlord ID: " + (error.response?.data?.message || error.message));
      } finally {
        setLoadingLandlordId(false);
      }
    };
 ...
 useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        ...
        // API returns data under 'dues' key
  ...
  useEffect(() => {
    const selectedRoom = properties
      .find((prop) => prop._id === formData.propertyId)
      ?.rooms?.find((room) => room.roomId === formData.roomId);
  ...
  }, [formData.roomId, formData.propertyId, properties]);
  useEffect(() => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "email") {
      if (!/^[^\s@]+@gmail\.com$/i.test(value.trim())) {
        errorMsg = "Please enter a valid Gmail address (must end with @gmail.com).";
      }
    }
  ...
  const validateForm = () => {
    ...
    if (!formData.email.trim() || !/^[^\s@]+@gmail\.com$/i.test(formData.email.trim())) {
      errors.email = "Please enter a valid Gmail address (must end with @gmail.com).";
      hasError = true;
    }
    if (!aadhaarVerified) {
              ...
              <form onSubmit={handleCategorySubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 block w-full px-3 py-2 text-black border border-teal-300 rounded-md focus:ring-teal-500 focus:border-teal-500"
                ...
                {categoryType === "fixed" && (
                  <div>
                    ...
                    <input
                      ...
                      className="mt-1 block w-full px-3 py-2 border border-teal-300 rounded-md focus:ring-teal-500 focus:border-teal-500 text-black"
...
const formatAadhaar = (value) => value.replace(/\D/g, '').replace(/(.{4})/g, '$1 ').trim();
