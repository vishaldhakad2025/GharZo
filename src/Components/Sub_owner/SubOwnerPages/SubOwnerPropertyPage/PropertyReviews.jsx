...
const PropertyFeedback = () => {
  ...
  const propertyId = paramPropertyId || localStorage.getItem("propertyId");
  const token = localStorage.getItem("token");

  console.log("Token:", token ? "Present" : "Missing");
  console.log("Property ID (from params or localStorage):", propertyId || "NOT FOUND");
  ...
  const fetchRatingsAndComments = async () => {
    try {
      ...
      if (!propertyId) {
        throw new Error("Property ID is missing from URL or localStorage.");
      }
      ...
      if (ratingsResponse.data.success && ratingsResponse.data.propertyStats) {
        ...
        setTotalRatings(0);
        setPropertyName("");
        console.warn("No ratings data available in response.");
      }
      ...
      console.log("Property ID used for comments fetch:", propertyId);
      ...
      if (commentsResponse.data.success && commentsResponse.data.comments) {
        ...
        console.warn("No comments data available in response.");
      }
      ...
      const errorMessage =
        error.response?.status === 401
          ...
          : error.message || "Failed to load ratings or comments. Please try again later.";
  ...
  useEffect(() => {
    ...
    if (!propertyId) {
      setError(`Property ID is missing (got: ${propertyId}). Check navigation or URL.`);
      setLoading(false);
      return;
    }

    fetchRatingsAndComments();
  }, [token, propertyId]); // Depend on propertyId too
