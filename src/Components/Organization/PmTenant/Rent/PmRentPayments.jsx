...
const TenantRentPayments = () => {
  ...
  const fetchAccommodationAndDues = async () => {
    try {
      ...
      const accommodation = res.data.accommodations?.[0];
      if (!accommodation) {
        throw new Error("No accommodation found for the tenant.");
      }

      const lId = accommodation.landlordId;
      const pName = accommodation.propertyName || "Property";
      const rId = accommodation.roomId || "Room";
      const pId = accommodation.propertyId;
      ...
      const formattedHistory = allBills.map((bill) => {
        ...
        const payMethod = bill.paymentMode || "—";
        const payDate = bill.status === "PAID" ? new Date(bill.updatedAt).toISOString().split("T")[0] : new Date(bill.dueDate).toISOString().split("T")[0];
        const payId = bill.razorpayPaymentId || null;
      ...
      setError(error.message || "Failed to load dues. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReceiptClick = async (paymentId) => {
    try {
      ...
      console.log("Receipt API Response:", res.data);

      if (res.data.success) {
        const payment = res.data.payment; // Main payment object

        const mergedData = {
          receiptNo: payment.receiptNumber || 'N/A',
          ...
          totalPaid: payment.totalAmount,   // Correct
          note: payment.note || 'Captured via Razorpay',
        };
      ...
      showCustomDialog('error', "Error fetching receipt details. Please try again.");
    ...
    <>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-6 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Accommodation Summary Card with Refresh Button */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <motion.button
                ...
                className="flex items-center gap-1 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 transition-colors"
              >
                        ...
                        <div className="flex gap-2">
                          {rent.status === "PENDING" && (
                            <motion.button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gradient-to-r from-green-400 via-cyan-400 to-blue-500 text-white text-xs font-medium shadow-md hover:shadow-lg transition-all duration-200"
                            >
        ...
        >
          ...
          >
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={closeDialog}
              className="absolute top-3 right-3 p-1.5 text-gray-400 hover:text-gray-600 rounded-full bg-white/80 shadow-md transition-colors"
            >
            ...
            <div className="text-center space-y-4">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={closeDialog}
                className="w-full px-4 py-2 bg-gradient-to-r from-green-400 via-cyan-400 to-blue-500 text-white rounded-xl font-medium shadow-md hover:shadow-lg transition-all duration-200"
              >
                OK
              </motion.button>
            </div>
      ...
      {showReceipt && currentReceipt && (
        ...
        >
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-white rounded-2xl max-w-md w-full max-h-[85vh] overflow-y-auto relative shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <motion.button
              whileHover={{ scale: 1.1 }}
            ...
            <div className="p-6 space-y-6">
              <div className="text-center border-b pb-4">
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="mx-auto w-12 h-12 bg-gradient-to-r from-green-400 via-cyan-400 to-blue-500 rounded-xl flex items-center justify-center mb-3"
                >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 p-4 bg-green-50 rounded-xl">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className={`font-bold ${currentReceipt.status === 'PAID' ? 'text-red-600' : 'text-green-600'}`}>
                ...
                <div className="p-4 bg-green-50 rounded-xl border border-green-100">
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-green-200">
                      ...
                      <span className="text-2xl font-bold text-green-600">{formatAmount(currentReceipt.totalPaid)}</span>
                    </div>
                  </div>
                </div>
