...
const Dues = () => {
 ...
 useEffect(() => {
  const fetchProfile = async () => {
    ...
    // Check landlord ID from localStorage
    ...
    try {
      ...
      if (response.ok) {
        ...
        const landlordIdFromProfile = data.id;
        setLandlordId(landlordIdFromProfile);
        setOrgProfile(data);
        setError(null);
        setLoading(false);
      } else {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[#374151]">
            <div>
              ...
              <span className="ml-2 px-3 py-1 bg-[#e7eaff] rounded-full">
                {orgProfile.organizationName || "N/A"}
              </span>
            </div>
            <div>
              <span className="font-medium">Email:</span>
              <span className="ml-2 px-3 py-1 bg-[#e7eaff] rounded-full">
                {orgProfile.email || "N/A"}
              </span>
            </div>
          </div>
        </div>
