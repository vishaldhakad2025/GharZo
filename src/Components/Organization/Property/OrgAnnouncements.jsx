...
const OrgAnnouncements = () => {
  ...
  // Change token getter to use orgToken
  const getAuthToken = () => localStorage.getItem('orgToken') || null;
  ...
  // Yahan title ko clean kar ke daalo (agar purana data mein symbol ho to)
  ...
  if (announcement.propertyId) {
    fetchTenantsByProperty(announcement.propertyId);
  }
  ...
  if (name === 'title') {
    ...
    const filteredValue = value.replace(/[^a-zA-Z\s]/g, '').replace(/\s+/g, ' ').trimStart();
  ...
  return (
    <AnimatePresence>
      <div className="max-w-4xl mx-auto p-2 sm:p-4 bg-white shadow-xl rounded-2xl min-h-screen flex flex-col">
        <ToastContainer />
        <div className="mb-8">
                  ...
                  {propertiesLoading ? (
                    ...
                    properties.map((property) => (
                      <option key={property._id} value={property._id}>
                        {property.name || property.propertyName} ({property._id})
                      </option>
                    ))
                  )}
                </select>
