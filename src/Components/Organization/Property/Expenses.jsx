...
const OrganizationExpenses = () => {
  ...
  const [categorySummaryMonth, setCategorySummaryMonth] = useState('10');
  ...
  // Fetch organization profile to get organizationId
  useEffect(() => {
  ...
  useEffect(() => {
    if (!organizationId || !orgToken) return;
    ...
    const fetchMonthlyExpenses = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setError(`Failed to fetch monthly expenses: ${errorData.message || 'Unauthorized'}`);
        }
      } catch (error) {
    ...
    const fetchYearlyExpenses = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setError(`Failed to fetch yearly expenses: ${errorData.message || 'Unauthorized'}`);
        }
      } catch (error) {
    ...
    fetchYearlyExpenses();
  }, [orgToken, organizationId, filters, year]);

  // Fetch category summary with year and month filters
  useEffect(() => {
    if (!orgToken || !organizationId || !categorySummaryYear || !categorySummaryMonth) {
  ...
  }, [orgToken, organizationId, categorySummaryYear, categorySummaryMonth]);

  const fetchCategoryById = async (categoryId) => {
    if (!orgToken || !organizationId || !categoryId) {
      setError('Missing token, organization ID, or category ID.');
      return;
    }
    try {
      const response = await fetch(
      ...
      if (response.ok || response.status === 304) {
        ...
        const errorData = await response.json();
        setError(`Failed to fetch category: ${errorData.message || 'Unauthorized'}`);
      }
    } catch (error) {
  ...
  const fetchExpenseById = async (expenseId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(`Failed to fetch expense: ${errorData.message || 'Unauthorized'}`);
        return null;
      }
    } catch (error) {
  ...
  const handleUpdateCategory = async () => {
    if (!organizationId || !orgToken || !categoryName || !editCategoryId) {
      setError('Please provide Organization ID, Token, Category Name, and Category ID');
      return;
    }
    try {
      const response = await fetch(
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(`Failed to update category: ${errorData.message || 'Unauthorized'}`);
      }
    } catch (error) {
  ...
  const handleAddCategory = async () => {
    if (!organizationId || !orgToken || !categoryName) {
      setError('Please provide Organization ID, Token, and Category Name');
      return;
    }
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(`Failed to add category: ${errorData.message || 'Unauthorized'}`);
      }
    } catch (error) {
  ...
  const handleDeleteCategory = async (categoryId) => {
    if (!organizationId || !orgToken || !categoryId) {
      setError('Please provide Organization ID, Token, and Category ID');
      return;
    }
    try {
      const response = await fetch(
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(`Failed to delete category: ${errorData.message || 'Unauthorized'}`);
      }
    } catch (error) {
  ...
  const handleUpdateExpense = async () => {
    if (!orgToken || !organizationId || !editExpenseId) {
      setError('Missing token, organization ID, or expense ID.');
      return;
    }
    if (
      !expenseData.category ||
  ...
  const openEditExpenseModal = (expense) => {
    setExpenseData({
      ...
      date: expense.date ? new Date(expense.date).toISOString().split('T')[0] : '2025-10-10',
  ...
  const uniquePaidBy = [...new Set(expenses.map((expense) => expense.paidBy))].filter(Boolean);
  const uniquePaidTo = [...new Set(expenses.map((expense) => expense.paidTo))].filter(Boolean);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-1 lg:col-span-1">
                    ...
                    <div className="space-y-4 max-h-64 overflow-y-auto">
                      {categorySummary && categorySummary.summary && categorySummary.summary.length > 0 ? (
