...
import RegionalManagerRoomOverview from "./RegionalManagerPropertyPage/RegionalManagerRoomOverview";
import RegionalManagerTenant from "./RegionalManagerPropertyPage/RegionalManagerTenant";
import RegionalManagerCollections from "./RegionalManagerPropertyPage/RegionalManagerCollections";
import RegionalManagerDues from "./RegionalManagerPropertyPage/RegionalManagerDues";
import RegionalManagerExpenses from "./RegionalManagerPropertyPage/RegionalManagerExpenses";
import RegionalManagerMaintenance from "./RegionalManagerPropertyPage/RegionalManagerMaintenance";
import RegionalManagerPropertyReviews from "./RegionalManagerPropertyPage/RegionalManagerPropertyReviews";
...
const RegionalManagerPropertyDetail = () => {
  const { propertyId } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const state = location.state;
  const type = state?.type || 'my';
  ...
  useEffect(() => {
    const fetchPropertyDetails = async () => {
      try {
        ...
        if (type === 'assign') {
          // Fetch assigned properties list and find the specific one
          response = await fetch(`${API_BASE}/api/regional-managers/assigned-properties`, {
            method: "GET",
            headers: {
          ...
          if (response.ok && data.success && data.data) {
            const foundProperty = data.data.find(p => p._id === propertyId);
            if (!foundProperty) {
          ...
          // Fetch single property for 'my' type
          response = await fetch(`${API_BASE}/api/rm/properties/${propertyId}`, {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        const adaptedProperty = {
          ...propertyData,
          stats: {
            totalRooms: propertyData.metrics?.totalRooms || propertyData.totalRooms || 0,
            totalBeds: propertyData.metrics?.totalBeds || propertyData.totalBeds || 0,
            monthlyCollection: propertyData.metrics?.monthlyCollection || propertyData.monthlyCollection || 0,
            occupancyRate: propertyData.metrics?.bedOccupancyRate || propertyData.metrics?.roomOccupancyRate || (propertyData.occupiedSpace && propertyData.totalCapacity ? (propertyData.occupiedSpace / propertyData.totalCapacity * 100).toFixed(2) : 0),
          },
          location: {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                {/* <p className="mb-3 text-gray-700">
                  ...
                  <span>{property?.stats?.occupancyRate || "N/A"}%</span>
                </p> */}
      ...
      case "Rooms":
        return <RegionalManagerRoomOverview propertyId={propertyId} />;

      case "Dues":
        return (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50  rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerDues propertyId={propertyId} />
          </div>
        );

      case "Tenant":
        return (
          <div className="p-2 rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerTenant propertyId={propertyId} />
          </div>
        );
      case "Complaints":
        return (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-0 rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerComplaints propertyId={propertyId} />
          </div>
        );
      case "Expenses":
        return (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-0 rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerExpenses propertyId={propertyId} />
          </div>
        );
      case "Maintenance":
        return (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-0 rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerMaintenance propertyId={propertyId} />
          </div>
        );
      case "Collections":
        return (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-0 rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerCollections propertyId={propertyId} />
          </div>
        );
      case "PropertyReviews":
        return (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-0 rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerPropertyReviews propertyId={propertyId} />
          </div>
        );
      case "Announcements":
        return (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-8 rounded-2xl border border-purple-200/50 shadow-lg">
            <RegionalManagerAnnouncement propertyId={propertyId} />
          </div>
        );
      default:
        return (
  ...
  if (!property) {
    return (
      <div className="flex justify-center items-center h-screen bg-gradient-to-br from-purple-900 to-pink-900">
        <div className="text-xl text-white font-bold">Property not found</div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="mt-12 flex justify-between items-center">
          ...
          {/* <motion.button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-gray-500 to-gray-700 text-white rounded-2xl hover:from-gray-600 hover:to-gray-800 shadow-lg transition-all duration-300"
...
export default RegionalManagerPropertyDetail;